...
- **Player-pair scheduling constraints** (Admin → session → **Manage player constraints**) — hard rules on top of targets and blackout dates: "these two players must never play the same week" or "these two players must always play the same week," entered per pair for that session. A never-together pair is kept apart regardless of team, opposing side, or court — it just means never scheduled the same week at all; an always-together pair needs equal targets, since they always play (and sit out) together. A constraint that's mathematically impossible for the roster (e.g. two never-together players whose targets alone add up to more weeks than the season has, or an always-together pair with different targets) is caught and explained rather than silently ignored the next time you (re)generate the schedule. Only applies to season scheduling — a manual Reassign or a self-service sub/swap doesn't check these.
...
- **"My Page"** (`/me`) — a bookmarkable, per-player dashboard: upcoming matches and status across every session they're in, ball duty weeks, one-click "Need a sub," a callout for any session still awaiting their blackout dates, a quick target/played/ball-duty stat line and any relevant blackout dates for each session, and their calendar-subscribe link, all in one place instead of switching sessions on each separate page. Reachable from the nav bar (each player gets a friendly, name-based URL rather than a bare numeric ID once they've been added), or automatically linked from the confirm/need-sub/claim-sub result pages and the footer of reminder/follow-up emails.
- **Public Player Stats page** (`/stats`) — a read-only page for players, one table per active session, showing every player's target games, games actually played, any bonus games picked up from subbing in, and ball duty count for the season so far. Every name links to that player's own My Page. No login needed — everything on it is already derivable from the public schedule anyway, just gathered in one place instead of counted by hand.
- **Games-won leaderboard** (`/scores`, `/leaderboard`) — a fun, informal "who's on top" tracker, separate from the fairness-scheduling/target system. Once a match has actually happened, a player can self-report how many games they personally won that week on their own bookmarkable Scores page (linked from the nav, the schedule/look-ahead pages, and My Page) — editable for 24 hours after they first save it, after which only an admin can change it (a "Games won" field is always available to an admin on the session detail page, no time limit). The Leaderboard, one per session, ranks total games won for the season, with matches scored and average per match shown alongside.
...
- **Dark mode, adjustable text size, and a mobile-friendly layout**: a toggle in the top-right of every page (public and admin) switches the whole site between light and dark, remembering your choice per-browser and otherwise following your system's light/dark setting the first time you visit. A **Preferences** page (linked in the nav, no login needed) lets anyone bump up text size, or turn on a higher-contrast large-text mode, independent of dark mode — also remembered per-browser. On a phone-width screen, the top nav collapses into a hamburger menu instead of running off the edge of the screen.
- **Weather forecast** — turn it on per session (a checkbox plus a latitude/longitude pair on the session's Edit page) and a small forecast — temperature, condition, chance of rain — shows up next to each upcoming date on the public schedule and look-ahead pages, on the admin session detail page, and right in the player's confirmation reminder email (or the ad-hoc "you're in" email). Pulled from OpenWeatherMap's free forecast API using one server-wide API key, refreshed automatically roughly once an hour starting once that week's first email goes out and continuing until an hour after match time. An **Update weather now** button on each week's card fetches it immediately instead of waiting on the automatic pass; a session with a bad API key or bad coordinates gets a throttled failure logged to the Activity Log (once an hour, not every retry) instead of failing silently or flooding it.
...
**Full Season Schedule** — every date, teams, and ball duty, plus a per-date weather forecast when a session has it turned on
<img src="src/public/img/help/full-schedule-weather.png" alt="Full Season Schedule page with weather forecast" width="100%">

</td>
<td width="50%">

**Next 4 Weeks** — the same table trimmed to just what's coming up soon
...
**Player Stats** — target vs. played vs. ball duty, for every player, at a glance
<img src="src/public/img/help/player-stats.png" alt="Player Stats page" width="100%">
