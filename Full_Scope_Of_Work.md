...
## 23. Ad-hoc sessions: resolved 2026-08-12

A second, fundamentally different session shape, requested alongside GitHub repo setup ("I'd like to talk through the requirements for this ad-hoc session as it's unlike a regular session"). Rather than building anything up front, this went through several rounds of requirements discussion before any code: first the broad shape (recurring but no fairness math, no blackout dates, no confirm/sub flow, roster picked fresh each week, dashboard kept visually separate), then Kyle described his actual real-world routine in detail — an invite email ~56 hours before match time, first 4 to respond play, a 5th starts a new group waiting on 3 more, sometimes 8+ sign up before a reminder is needed, a reminder ~30 hours out if sign-ups aren't a clean multiple of 4, and a final "who's playing" email ~24 hours out — explicitly "no connection to the blackout dates, double books, already scheduled playing times." A last clarifying round settled the remaining ambiguities: the invite list is the active roster but editable per ad-hoc session; a leftover incomplete group at the final cutoff is told "not enough signed up" automatically; the three lead times are adjustable per session (not hardcoded); the 30-hour reminder goes only to players who haven't signed up yet.

**What got built**, matching that exact routine: `sessions.session_type` (`'regular'`/`'adhoc'`, fixed at creation) plus three per-session lead-hour fields. A new `adhoc_signups` table tracks a hashed sign-up token and a `signed_up_at` timestamp per roster player per week — that timestamp is the first-come-first-served order, live-chunked into complete groups of 4 (never stored) by `adhocFlow.js`'s `courtGroupsForWeek()`. Three new cron passes (`processAdhocInvites`/`processAdhocReminders`/`processAdhocFinalization`) handle the three touchpoints; finalization materializes every complete group into real `week_assignments` rows (status `'confirmed'` directly, since signing up already is the confirmation) and emails either "here's your court" or "not enough signed up" to whoever's left over. Four new email templates, a dedicated admin session-detail view (separate from the regular one rather than threading conditionals through it), a session-type toggle on the New Session form, and a separate "Ad-hoc sessions" section on the dashboard. Ad-hoc sessions are explicitly excluded from Request a Sub, Swap a Week, and the blackout-dates page (nothing to sub/swap out of, no blackout concept) but do show up on the public schedule, My Page, calendar, and PDF once a court finalizes, same as a regular match — just with no sub/swap actions on it.

Verified end-to-end in a live sandbox: a full invite → signup → stragglers-only reminder → finalize cycle with 9 roster players produced exactly the right court groupings and email counts at every stage (9 invites, 3 reminders to only the non-signed-up players, 4 finalized `week_assignments` all `'confirmed'`, 4 final-roster emails, 3 not-enough emails), a second finalization pass sent zero further emails (idempotent), and the real admin routes (New Session form, session creation, Edit page with the type locked and roster pre-checked, dashboard's new section) were exercised through actual HTTP requests against a running server, alongside confirming a regular session's creation and the public `/schedule` page still work with no regressions. See CLAUDE.md's "Ad-hoc sessions" section for full implementation detail, including a noted gap: no dedicated UI yet to edit who's on a court once a week has finalized.
